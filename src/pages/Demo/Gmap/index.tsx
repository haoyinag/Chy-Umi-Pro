...
const getMenu = (map2: any, callBack: Function) => {
  let menu_content = [];

  menu_content.push("<div class='info context_menu'>");
  menu_content.push(`<p ${(onclick = () => callBack && callBack())}>缩小</p>`);
  menu_content.push(
    "  <p class='split_line' onclick='map2.zoomMenu(1)'>放大</p>",
  );
  menu_content.push(
    "  <p class='split_line' onclick='map2.distanceMeasureMenu()'>距离量测</p>",
  );
  menu_content.push("  <p onclick='map2.addMarkerMenu()'>添加标记</p>");
  menu_content.push('</div>');

  return menu_content.join('');
};
...
export default (): JSX.Element => {
  ...
  useEffect(() => {
    if (map) {
      ...
      //   map.plugin(["AMap.MouseTool"], (e: any) => {
      //     console.log(e);
      //   });
  ...
  useEffect(() => {
    if (editIndex || editIndex === 0) {
      ...
      setPolyEditor(null);
      /** 多边形弹窗必须在插件AMap.PolygonEditor中实现 */
